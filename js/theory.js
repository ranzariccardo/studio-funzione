...
const COMMON = {
  symmetry: 'Perché conviene saperlo: se la funzione è pari o dispari, il grafico è simmetrico (rispetto all’asse y se pari, rispetto all’origine se dispari) — basta studiare metà dominio (x ≥ 0) e poi riflettere per ottenere l’altra metà, dimezzando il lavoro nei passi successivi (segno, derivate, grafico). Come si calcola, in sintesi: prima si verifica che il dominio sia simmetrico rispetto all’origine (altrimenti la funzione non può essere né pari né dispari, e il test si ferma qui). Poi si calcola f(-x) e si confronta con f(x): se f(-x) = f(x) è pari; se f(-x) = -f(x) è dispari; altrimenti non è né pari né dispari.',
  intersections: 'Intersezione con l’asse y: si calcola f(0), se x = 0 appartiene al dominio — è il punto (0, f(0)). Intersezioni con l’asse x: si risolve f(x) = 0 — sono gli zeri della funzione, i punti dove il grafico tocca o attraversa l’asse x.',
...
export const RATIONAL_THEORY = {
  ...
  intersections: COMMON.intersections,
